...
class SemiLocalIMEDRL(ContextualIMEDRL):
    ...
    def _init_statistics(self):
        # Local: indexed by (x, s, a)
        self.state_action_pulls = np.zeros((self.nX, self.nS, self.nA), dtype=int)
        self.state_visits = np.zeros((self.nX, self.nS), dtype=int)
        self.rewards = np.full((self.nX, self.nS, self.nA), 0.5)
        self._all_selected = np.zeros((self.nX, self.nS), dtype=bool)
        self._phi = np.zeros((self.nX, self.nS))
        self.rewards_distributions = {
            x: {s: {a: {1: 0, 0.5: 1} for a in range(self.nA)} for s in range(self.nS)}
    ...
    def get_all_selected(self, observation):
        x = self.get_context(observation)
        s = self.get_state(observation)
        return self._all_selected[x, s]
 
    def set_all_selected(self, observation, value):
        x = self.get_context(observation)
        s = self.get_state(observation)
        self._all_selected[x, s] = value
 
    def get_pull_count(self, observation, action):
        x = self.get_context(observation)
        s = self.get_state(observation)
        return self.state_action_pulls[x, s, action]
 
    def get_reward_estimate(self, observation, action):
        x = self.get_context(observation)
        s = self.get_state(observation)
        return self.rewards[x, s, action]
    ...
    def get_reward_distribution(self, observation, action):
        x = self.get_context(observation)
        s = self.get_state(observation)
        return self.rewards_distributions[x][s][action]
 
    def get_reward_from_state_action(self, observation, state, action):
        x = self.get_context(observation)
        return self.rewards[x, state, action]
    ...
    def update_statistics(self, observation, action, reward, next_observation):
        ...
        # Local reward update
        na_r = self.state_action_pulls[x, s, action]
        r = self.rewards[x, s, action]
 
        self.state_action_pulls[x, s, action] = na_r + 1
        self.state_visits[x, s] = self.state_visits[x, s] + 1
        self.rewards[x, s, action] = ((na_r + 1) * r + reward) / (na_r + 2)
 
        rd = self.rewards_distributions[x][s][action]
        if reward in rd:
            rd[reward] += 1
        else:
            rd[reward] = 1
