...
class ETC(ContextualAgent):
    ...
    """
    def __init__(self, nS, nA, nX, skeleton=None, learning_scope="global", name="ETC_Base"):
        super().__init__(nS, nA, nX, learning_scope=learning_scope, name=name)
        ...
        if self.uses_context_for_rewards():
            ...
            self.rewards = np.zeros((self.nX, self.nS, self.nA)) + 0.5
        else:
        ...
        if self.uses_context_for_transitions():
            self.transitions = np.ones((self.nX, self.nS, self.nA, self.nS)) / self.nS
            self.transition_pulls = np.zeros((self.nX, self.nS, self.nA), dtype=int)
        else:
...
class GlobalETC4(ETC):
    ...
    def stop_exploration(self):
        """
        Stops strictly when the total number of exploratory actions taken reaches 3.
        """
